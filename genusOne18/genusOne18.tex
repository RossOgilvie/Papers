...
\]
% There are several topological implications of the previous lemma. First, each level set $\mathcal{\tilde{A}}(p,q)$ is path connected. More precisely each of these is diffeomorphic to a product of $(0,1)$ and $\R$. Therefore the above disjoint union is the decomposition of the preimage into its path connected components. 
% By varying the value of $q$, we see that the $\mathcal{\tilde{A}}(p,q)$ foliate $\mathcal{\tilde{A}}(p)$, so $\mathcal{\tilde{S}}$ is arranged densely in $\mathcal{\tilde{A}}(p)$, analogously to how $\Q$ is arranged densely in $\R$.
...
%  (as shown in Figure~\ref{fig:circle rotation}).
...
As $u$ and $v$ are both positive, it must be that $\tilde{u} \in (2\pi n, 2\pi n + \pi)$ and $\tilde{v} \in (2\pi m, 2\pi m + \pi)$ for some integers $n$ and $m$. Hence $\tilde{U}$ and $\tilde{V}$ also lie in those two intervals respectively. Under applying the transformation $\tilde{\lambda}$, the coordinate $\tilde{U}$ will be translated by $\pi$ and so lie in $(2\pi(n+1) - \pi, 2\pi(n+1))$. In other words its winding number has increased by $1$. The same can be said for $\tilde{V}$. As $\tilde{u}$ and $\tilde{v}$ have the same winding numbers as $\tilde{U}$ and $\tilde{V}$, combining the effect of $\lambda$ on $T_0$ with the change of winding number in \eqref{eqn:tilde T computable} shows proves the relation on this particular subset of $\mathcal{\tilde{A}}$. But a relation between analytic functions on an open set applies everywhere, by continuation.
...
% \[
% \frac{n\Gamma^- - m \Gamma^+}{gm}
% = \frac{n n'mx - m n'my}{m'm}
% = \frac{n'(nx - my)}{m'}
% = \frac{n'}{m'},
% \]
% So we can solve~\eqref{eqn:period differential scaling} for a differential $\psi^P$ with period $2\pi\iu g$ and integrals $2\pi\iu \Gamma^+, 2\pi\iu \Gamma^-$.
...
\begin{align*}
% \frac{n'}{m'} &= \frac{n\Gamma_- - m\Gamma_+}{l'm} \\
